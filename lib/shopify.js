 ...
 *  - Every read helper falls back to the mock catalog in /app/lib/products.js
 *    so the site keeps rendering while the merchant is still populating Shopify.
...
import { PRODUCTS as MOCK_PRODUCTS, getProductsForCollection as mockGetProductsForCollection, getProductByHandle as mockGetProductByHandle, COLLECTIONS as MOCK_COLLECTIONS } from '@/lib/products';
...
// -----------------------------------------------------------
// Safe read helpers with fallback to mock
// -----------------------------------------------------------
export async function getAllProductsLive({ first = 50 } = {}) {
  if (!shopifyEnabled) return { products: MOCK_PRODUCTS, source: 'mock' };
  try {
    const data = await shopifyFetch(PRODUCTS_QUERY, { first }, { cache: 'no-store' });
    const nodes = (data.products?.edges || []).map((e) => e.node);
    if (!nodes.length) return { products: MOCK_PRODUCTS, source: 'mock' };
    return { products: nodes.map(adaptProduct), source: 'shopify' };
  } catch (e) {
    console.warn('[shopify] getAllProductsLive failed, falling back to mock:', e.message);
    return { products: MOCK_PRODUCTS, source: 'mock' };
  }
}

export async function getProductByHandleLive(handle) {
  if (!shopifyEnabled) return { product: mockGetProductByHandle(handle), source: 'mock' };
  try {
    const data = await shopifyFetch(PRODUCT_BY_HANDLE_QUERY, { handle }, { cache: 'no-store' });
    if (data.product) return { product: adaptProduct(data.product), source: 'shopify' };
    return { product: mockGetProductByHandle(handle), source: 'mock' };
  } catch (e) {
    console.warn('[shopify] getProductByHandleLive failed, falling back to mock:', e.message);
    return { product: mockGetProductByHandle(handle), source: 'mock' };
  }
}

export async function getCollectionProductsLive(handle, { first = 24 } = {}) {
  if (!shopifyEnabled) return { products: mockGetProductsForCollection(handle), source: 'mock' };
  try {
    ...
    if (!nodes.length) return { products: mockGetProductsForCollection(handle), source: 'mock' };
    return { products: nodes.map(adaptProduct), source: 'shopify', title: data.collection?.title };
  } catch (e) {
    console.warn('[shopify] getCollectionProductsLive failed, falling back to mock:', e.message);
    return { products: mockGetProductsForCollection(handle), source: 'mock' };
  }
}
...
export const _mock = { MOCK_PRODUCTS, MOCK_COLLECTIONS };
