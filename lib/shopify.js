...
import { AVAILABILITY_META } from '@/lib/products';
 ...
 *
 * Uses the Storefront API's customerCreate mutation (no Admin API scopes
 * required). We generate a random secure password server-side so the customer
 * can later "Reset password" to claim their account if they wish. Setting
 * `acceptsMarketing: true` makes them appear in Shopify Admin → Customers →
 * "Email subscribers" segment, ready for Shopify Email campaigns.
 */
export async function subscribeToNewsletter(email) {
  ...
  // 32 random alphanumeric chars — meets Shopify's password requirements and is
  // effectively unrecoverable; user can reset via Shopify's "Forgot password".
  ...
  if (taken) {
    // Idempotent success — user is already in the customer list. Their marketing
    // consent might be off; the merchant can toggle it in Shopify Admin, or we
    // can add a Customer Account API update flow later.
