...
const ALL_SOCIALS = [
  ...
  { Icon: TiktokIcon,    href: SITE_CONFIG.social.tiktok,    label: 'TikTok',    color: 'hover:text-white' },
  { Icon: Github,        href: SITE_CONFIG.social.github,    label: 'GitHub',    color: 'hover:text-white' },
];
  ...
  return (
    <footer className="relative overflow-hidden border-t border-red-900/30">
      ...
      <div className="container mx-auto px-6 pt-16 pb-8 relative z-10">
        <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="flex justify-center mb-14">
          <BackToTop />
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-14">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="lg:col-span-2 space-y-5">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {SOCIALS.map((s) => (
                <motion.a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label} whileHover={{ y: -3, scale: 1.05 }} transition={{ type: 'spring', stiffness: 400 }} className={`w-10 h-10 rounded-lg glass-panel flex items-center justify-center text-neutral-400 ${s.color} transition-colors border border-neutral-800 hover:border-red-700/60`}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.2 }}>
            ...
            <ul className="space-y-3">
              {CONNECT_LINKS.filter((l) => !!l.href && !l.href.endsWith(':')).map((l) => (
                <li key={l.label}>
                  <a href={l.href} target={l.external ? '_blank' : undefined} rel={l.external ? 'noopener noreferrer' : undefined} className="relative inline-block text-neutral-400 hover:text-white text-sm transition-colors group">
                    <span className="relative z-10">{l.label}</span>
                    <span className="absolute -bottom-1 left-0 w-0 h-px bg-red-500 group-hover:w-full transition-all duration-500 shadow-[0_0_8px_rgba(220,38,38,0.8)]" />
                  </a>
                </li>
              ))}
              {['Twitter', 'Instagram', 'TikTok'].filter((name) => !SITE_CONFIG.social[name.toLowerCase()]).map((name) => (
